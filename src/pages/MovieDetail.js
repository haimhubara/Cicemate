...
const MovieDetail = () => {
  ...
  useEffect(()=>{
    async function fetchMovie(){
      const respone = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=${process.env.REACT_APP_API_KEY}`);
  ...
  const imageUrl = movie.poster_path ? `https://image.tmdb.org/t/p/w500/${movie.poster_path}` :backupImg;


  return (
        <main>
          <section className="flex justify-start flex-wrap py-5 ">
            <div className="max-w-sm flex-shrink-0 lg:ml-28 mr-20">
              <img className="rounded" src={imageUrl} alt ={movie.title}></img>
             
            </div>

            <div className="max-2-2xl text-gray-700 text-lg dark:text-white">
              <h1 className="text-4xl font-bold my-3 text-center lg:text-left">{movie.title}</h1>
              <p className="my-4  max-w-xl" >{movie.overview}</p>
